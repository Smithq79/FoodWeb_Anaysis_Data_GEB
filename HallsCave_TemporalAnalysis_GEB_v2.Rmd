...
```{r}
interval_dat=read.csv("New_Data/Time_Intervals_introduced_4-2-2024.csv", check.names = FALSE)
#interval_dat_introduced=read.csv("New_Data/Time_Intervals_3-9-2023.csv", check.names = FALSE)
climate_dat=read.csv("New_Data/Halls Cave Climate Data for 16 Time Intervals.csv", check.names = FALSE)

names(climate_dat) <- iconv(names(climate_dat), to = "ASCII", sub = "")

mat_raw=as.matrix(read.csv("New_Data/Matrix_4-6-2023.csv", row.names=1))

spp_attrs=read.csv("New_Data/spp_attributes_4-20-2023.csv")

plei_sites=read.csv("New_Data/Pleistocene_Sites_3-9-2023.csv")

holo_sites=read.csv("New_Data/Holocene_Sites_3-9-2023.csv")
```
...
***Let's save this version of the matrix for use as supplemental data***
```{r}
write.csv(mat, "Supplemental Data/full_matrix.csv", row.names = rownames(mat))
```
...
***Let's save this as an rds file so that we can just start the supplemental materials from here.

```{r}
save(time.list, file="Supplemental Data/species_lists.rdata")
```
...
### 4.2. Build Bipartite Network

Making bipartite networks based on pre-assigned trophic category (**OLD**)
*We are removing this and building the bipartite network using an alternative categorization of omnivores. See below*
```{r}
# bipart.mats=list()
# spp_counts=vector()
...
# 
# resource.rows=which(interval_dat_list[[k]]$Trophic_Category=="browser"|interval_dat_list[[k]]$Trophic_Category=="grazer"|interval_dat_list[[k]]$Trophic_Category=="mixed feeder"|interval_dat_list[[k]]$Trophic_Category=="frugivore_granivore"|interval_dat_list[[k]]$Trophic_Category=="insectivore"|interval_dat_list[[k]]$Trophic_Category=="omnivore")
# 
...
### 4.3. Build Bipartite Network (**NEW**)
...
Do this for all intervals
```{r, fig.width=12, fig.height=15}
par(mfrow=c(4,2), mar=c(1,1,1,1))
for(i in 1:length(time_nets)){
 plotweb(bipart.mats[[i]], method="normal",ybig=0.1, col.interaction="black", arrow="no",  col.high="tomato", col.low="lightblue",labsize=1,  adj.low=c(1,0), adj.high=c(0,0.5), text.rot = 90, y.lim=c(-1,2.0))
}
```
...
NOS is very strongly correlated with connectance
```{r}
cor.test(sum.data$connectance, sum.data$NOS,method="spearman")
```
...and modularity is also negatively correlated with connectance
```{r}
cor.test(sum.data$connectance, sum.data$web.modularity,method="spearman")
```
...
```
Interestingly, connectance is NOT correlated with species richness (these are often correlated).
```{r}
...
```{r, fig.height=12}
plot_grid(mod.nos.plot_2, connectance.plot_2, int_diversity.plot, nrow=3, align="v")
```
...
Finally, here is how we scale up to get null distribution of Nbar values for all time intervals with x10000 permutations.
Let's also calculate connectance
```{r, warning=F, eval=F}
null.nos=list()
...
```{r, eval=F}
save(null.nos, file="null_nos.rdata")
save(null.connectance, file="null_connectance.rdata")
```


```{r}
load("null_nos.rdata")
load("null_connectance.rdata")
```
...
```{r}
sum.data$P.value_mods=unlist(p.list)
```
...
```
```{r}
#just raw precipitation across time
...
```
```{r}
#just raw species richness across time

spc.plot = ggplot(data=time.dat, aes(x=-interval.mid)) +
  geom_rect(xmin=interval.start, xmax=interval.end, ymin=-0.3, ymax=Inf, fill="gray", alpha=0.5) +
  geom_rect(xmin=interval.start[9], xmax=interval.end[9], ymin=-0.3, ymax=Inf, fill="#ffeda0", alpha=0.5) +
      geom_rect(xmin=interval.start[11], xmax=interval.end[11], ymin=-0.3, ymax=Inf, fill="#ffc675", alpha=0.5) +
  geom_vline(xintercept = 950.0, color = "white", size=1.5) +
...
plot_grid(mod.nos.plot_2, connectance.plot_2, temp.plot, precip.plot, spc.plot, nrow=5, align="v")
...
### 9.3. Tabulate Trophic Categories
...
### 9.4. Temporal autocorrelation plots
```{r}
library(tseries)
acf(time.dat$TM)
acf(time.dat.change$TM.change[1:15])

acf(time.dat$Tavg)
acf(time.dat.change$Tavg.change[1:15])

acf(time.dat$Pr)
acf(time.dat.change$Pr.change[1:15])

acf(time.dat$SpC)
acf(time.dat.change$SpC.change[1:15])
```
## References:
